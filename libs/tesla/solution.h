...
namespace tesla {
	...
	struct variable_name_comparator {
		bool operator()(const tesla::variable_name & a, const tesla::variable_name & b) const;
	};
	...
	public:
		...
		std::map<variable_name, unsigned int, variable_name_comparator> get_solved() const;

		void resolve_equations();

	private:
		std::map<variable_name, equation, variable_name_comparator> _equations;

		std::map<variable_name, unsigned int, variable_name_comparator> _solutions;
